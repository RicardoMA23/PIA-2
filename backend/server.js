...
// GET de diagnóstico (por si navegas)
  ...
  try {
    const q = `
      SELECT d.id_documento, d.codigo, d.nombre_documento, d.version, d.fecha,
             d.estado, d.url_archivo, COALESCE(u.nombre,'') AS responsable
...
app.post('/api/documentos', authMiddleware, upload.single('archivo'), async (req, res) => {
  try {
    const {
      codigo,                // opcional (si trigger lo genera)
      nombre_documento,
      version = 'v1.0',
      fecha,                 // opcional
      estado = 'En Revisión',
    ...
    const q = `
      ...
      RETURNING id_documento, codigo, nombre_documento, version, fecha, estado, url_archivo
    `;
    const params = [
      codigo || null, nombre_documento, version, fecha || null,
      id_responsable || null, estado, archivo, proceso || null
    ];
...
// Actualizar metadatos (sin cambiar archivo)
app.put('/api/documentos/:id', authMiddleware, async (req, res) => {
  try {
    ...
    `;
    const { rows } = await pool.query(q, [
      nombre_documento || null, version || null, fecha || null, estado || null,
      id_responsable || null, codigo || null, proceso || null, id
    ]);
...
// Reemplazar archivo
app.put('/api/documentos/:id/archivo', authMiddleware, upload.single('archivo'), async (req, res) => {
  try {
    const id = req.params.id;
    const archivo = req.file ? `/uploads/documentos/${req.file.filename}` : null;
    ...
    const prev = await pool.query('SELECT url_archivo FROM calidad.documentos WHERE id_documento=$1', [id]);
...
// Exportar 1 indicador a PDF sencillo (opcional)
app.get('/api/indicadores/:id/pdf', authMiddleware, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const { rows } = await pool.query(
      `SELECT * FROM calidad.indicadores WHERE id_indicador=$1 LIMIT 1`, [id]
    );
    if (!rows.length) return res.status(404).json({ error: 'No encontrado' });
    ...
    const kv = (k, v) => doc.fontSize(11).text(`${k}: `, { continued: true }).font('Helvetica-Bold').text(v ?? '').font('Helvetica');
    kv('Código', i.codigo);
    ...
    kv('Fecha deseada fin', i.fecha_deseada_finalizacion ? new Date(i.fecha_deseada_finalizacion).toLocaleDateString('es-MX') : '');
